...
export default function Onboarding() {
  ...
  // PRELOAD: corre una sola vez al entrar a Onboarding
  useEffect(() => {
    const urls = [hero1, hero2, hero3, overlayA, bgBase, facebookLogo, googleLogo, appleLogo, xLogo];
    urls.forEach((src) => {
      const img = new Image();
      img.src = src;
    });
    ...
    <div className="relative min-h-screen overflow-hidden bg-[#F5FBF3]">
      ...
      <div className="relative z-10 min-h-screen px-6 pt-10 pb-10 flex flex-col">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={prev}
            className={`inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/70 text-2xl text-zinc-900 transition ${
          ...
          <button onClick={finish} className="text-sm font-semibold text-zinc-700">
            Skip
          </button>
        </div>
